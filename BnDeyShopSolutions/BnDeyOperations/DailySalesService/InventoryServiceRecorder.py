...
class InventoryServiceRecorder:
    ...
    def create_daily_sale(self, brand, size_ml, brand_category, sale_date, qty):
        """
        Insert sale only if enough stock is available.
        If not enough stock, raise an exception and do not insert.
        """
        ...
        if available_qty < qty:
            raise ValueError(
                f"Insufficient stock for sale. "
                f"Available={available_qty}, Requested={qty}, "
                f"Product={brand} {size_ml}ML"
            )
    ...
    # ---------------------------------------------------------
    # 3. OPTIONAL HELPER TO INSERT INCOMING STOCK
    # ---------------------------------------------------------
    def add_incoming_stock(self, brand, size_ml, brand_category, stock_date, qty):
        qty = int(qty)

        if qty <= 0:
            raise ValueError("Incoming stock quantity must be greater than 0")
        ...
        result = self.incoming_stock.insert_one(doc)

        return {
            "status": "success",
            "message": "Incoming stock inserted successfully",
            "incoming_id": str(result.inserted_id)
        }
...
if __name__ == "__main__":
    service = InventoryServiceRecorder(
        mongo_uri="mongodb://localhost:27017/",
        db_name="inventory_db"
    )

    process_date = "09/04/2026"
    DailySale_sheet = '/Users/mehermeka/PycharmProjects/PythonProjectSelenium/chapter/BnDeyOperations/data/Daily_Sales/Daily_sales_09042026.csv'

    # Example: create sale only if stock is available
    try:

        sale_result = service.create_daily_sales_from_csv(DailySale_sheet)
        print(sale_result)
    except ValueError as e:
        print(f"Sale failed: {e}")

    # Example: end-of-day inventory update
    try:
        update_result = service.update_current_inventory_daily(process_date)
        print(update_result)
    except Exception as e:
